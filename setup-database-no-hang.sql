/*
  # Database Setup - Safe Version (No Hanging)
  This version checks for existing tables and handles conflicts
*/

-- ============================================
-- STEP 1: Create profiles table (without foreign key first)
-- ============================================

-- Drop table if exists to avoid conflicts (CAREFUL - this deletes data!)
...
-- Add foreign key constraint separately (only if table was just created)
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'farms_profile_id_fkey'
  ) THEN
    ALTER TABLE farms 
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE farms ADD CONSTRAINT farms_profile_id_key UNIQUE (profile_id);
  END IF;
END $$;
...
BEGIN
  ...
  INSERT INTO public.profiles (
    ...
    NEW.id, NEW.id, NEW.email, user_role, user_full_name, user_phone,
  ...
  IF user_role IN ('gardener', 'farmer', 'rancher') THEN
    INSERT INTO public.farms (
      profile_id, farm_type, property_size, current_challenges,
      garden_type, growing_zone, soil_type, sunlight_exposure,
      crop_types, certifications, equipment, farming_practices,
      livestock_types, herd_size, grazing_management, water_resources
    ) VALUES (
      NEW.id, user_role,
      COALESCE(
      ...
      CASE WHEN NEW.raw_user_meta_data->>'crop_types' IS NOT NULL 
        THEN string_to_array(NEW.raw_user_meta_data->>'crop_types', ',')
        ELSE NULL END,
      CASE WHEN NEW.raw_user_meta_data->>'certifications' IS NOT NULL 
      ...
      CASE WHEN NEW.raw_user_meta_data->>'livestock_types' IS NOT NULL 
